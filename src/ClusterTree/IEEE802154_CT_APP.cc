#include "IEEE802154_CT_APP.h"

Define_Module(IEEE802154_CT_APP);

void IEEE802154_CT_APP::initialize(int aStage)
{
    TrafGenPar::initialize(aStage);
    EV << getParentModule()->getFullName() << ": initializing IEEE802154_CT_APP, stage=" << aStage << endl;
    if (0 == aStage)
    {
        m_debug             = par("debug");
...
void IEEE802154_CT_APP::handleLowerMsg(cMessage* apMsg)
{
    simtime_t e2eDelay;
...
//***************************************************************
void IEEE802154_CT_APP::handleSelfMsg(cMessage *apMsg)
{
...
*/
void IEEE802154_CT_APP::SendTraf(cPacket* apMsg, const char* apDest)
{
    delete apMsg;
